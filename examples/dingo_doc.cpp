...
void test_default_create() {
  fs::remove_all("./temp");
  tantivy_logger_initialize("./log/", "info", false, tantivy_log_callback, true,
                            true);
  TantivySearchIndexRW *indexRW = tantivy_create_index2("./temp");
  tantivy_index_doc2(
      indexRW, 0, "Ancient empires rise and fall, shaping history's course.");
  tantivy_index_doc2(
      indexRW, 1, "Artistic expressions reflect diverse cultural heritages.");
  tantivy_index_doc2(
      indexRW, 2, "Social movements transform societies, forging new paths.");
  tantivy_index_doc2(indexRW, 3,
                     "Economies fluctuate, reflecting the complex interplay of "
                     "global forces.");
  tantivy_index_doc2(
      indexRW, 4, "Strategic military campaigns alter the balance of power.");
  tantivy_index_doc2(indexRW, 5,
                     "Quantum leaps redefine understanding of physical laws.");
  tantivy_index_doc2(indexRW, 6,
                     "Chemical reactions unlock mysteries of nature.");
  tantivy_index_doc2(indexRW, 7,
                     "Philosophical debates ponder the essence of existence.");
  tantivy_index_doc2(indexRW, 8,
                     "Marriages blend traditions, celebrating love's union.");
  tantivy_index_doc2(
      indexRW, 9,
      "Explorers discover uncharted territories, expanding world maps.");
  tantivy_index_doc2(indexRW, 10,
  ...
  tantivy_index_doc2(
      indexRW, 13, "Ancient philosophies provide wisdom for modern dilemmas.");
  tantivy_index_doc2(indexRW, 14,
                     "Economic theories debate the merits of market systems.");
  tantivy_index_doc2(
      indexRW, 15,
      "Military strategies evolve with technological advancements.");
  tantivy_index_doc2(indexRW, 16,
  ...
  int searched2 =
      tantivy_count_in_rowid_range2(indexRW, "redefi*", 0, 19, false);
  cout << "searched2:" << searched2 << endl;

  tantivy_commit2(indexRW);
  tantivy_free2(indexRW);
  // search
  TantivySearchIndexR *indexR = tantivy_load_index("./temp");
  // int searched = tantivy_count_in_rowid_range(indexR, "The cat sleeps as the
  // sun sets.", 0, 14);
...
void test_regx_create() {
  ...
  tantivy_index_doc2(
      indexRW, 19,
      "Wedding ceremonies acro%ss cultures symbolize lifelong commitment.");
  tantivy_index_doc2(indexRW, 20, "cultures");
  // tantivy_index_doc2(indexRW, 20, "commitment");

  int searched2 = tantivy_count_in_rowid_range2(indexRW, "%\\%%", 19, 19, true);
  cout << "searched2:" << searched2 << endl;
  ...
  // bool searched = tantivy_regex_count_in_rowid_range(indexR, ".*me.*", 20,
  // 20);
  cout << "searched:" << searched << endl;
...
void test_tokenizer_create() {
  ...
  tantivy_index_doc2(
      indexRW, 17,
      "在医学领域，化学化合物的作用至关重要，它们在许多重大医疗突"
      "破中扮演了核心角色。");
  tantivy_index_doc2(
      indexRW, 18,
      "当代哲学家在探讨人工智能时代的伦理道德问题，对机器与人类的"
      "关系进行深刻反思。");
  tantivy_index_doc2(
      indexRW, 19,
      "不同文化背景下的婚礼仪式代表着一生的承诺与责任，象征着两颗"
      "心的永恒结合。");
  tantivy_logger_initialize("./log/", "info", true, tantivy_log_callback, true,
                            true);
