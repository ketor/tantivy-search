...
void mylog(int level, std::string message) {
  switch (level) {
    case -2:  // -2 -> fatal
      if (log_level >= -2) std::cout << getCurrentDateTime() << " - FATAL " + message << endl;
      break;
    case -1:  // -1 -> error
      if (log_level >= -1) std::cout << getCurrentDateTime() << " - ERROR " + message << endl;
      break;
    case 0:  // 0 -> warning
      if (log_level >= 0) std::cout << getCurrentDateTime() << " - WARNING " + message << endl;
      break;
    case 1:  // 1 -> info
      if (log_level >= 1) std::cout << getCurrentDateTime() << " - INFO " + message << endl;
      break;
    case 2:  // 2 -> debug
      if (log_level >= 2) std::cout << getCurrentDateTime() << " - DEBUG " + message << endl;
      break;
    case 3:  // 3 -> tracing
      if (log_level >= 3) std::cout << getCurrentDateTime() << " - TRACING " + message << endl;
      break;
    default:
...
void tantivy_easylogging_callback(int level, const char* thread_id, const char* thread_name, const char* message) {
  ...
  switch (level) {
    case -2:  // -2 -> fatal
      if (log_level >= -2) LOG(FATAL) << "[" << threadId << ":" << threadName << "]" << msg;
      break;
    case -1:  // -1 -> error
      if (log_level >= -1) LOG(ERROR) << "[" << threadId << ":" << threadName << "]" << msg;
      break;
    case 0:  // 0 -> warning
      if (log_level >= 0) LOG(WARNING) << "[" << threadId << ":" << threadName << "]" << msg;
      break;
    case 1:  // 1 -> info
      if (log_level >= 1) LOG(INFO) << "[" << threadId << ":" << threadName << "]" << msg;
      break;
    case 2:  // 2 -> debug
      if (log_level >= 2) LOG(DEBUG) << "[" << threadId << ":" << threadName << "]" << msg;
      break;
    case 3:  // 3 -> tracing
      if (log_level >= 3) LOG(TRACE) << "[" << threadId << ":" << threadName << "]" << msg;
      break;
    default:
